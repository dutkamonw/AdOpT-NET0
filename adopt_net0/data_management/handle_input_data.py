                ...
                if netw_data.capex_defined_per_arc:
                    ...
                    for gamma in ["gamma1", "gamma2", "gamma3", "gamma4"]:
                        ...
                        if os.path.isfile(gamma_path):
                            gamma_per_arc[gamma] = pd.read_csv(
                                gamma_path, sep=";", index_col=0
                            )
                        else:
                            gamma_per_arc[gamma] = pd.DataFrame(
                                netw_data.economics.capex_data[gamma],
                                index=netw_data.distance.index,
                                columns=netw_data.distance.columns,
                            )
                    netw_data.gamma_per_arc = gamma_per_arc
